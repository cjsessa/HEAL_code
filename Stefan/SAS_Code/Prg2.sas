...
data pro2014; set t1.provider_aco_ssp_2014; run;
data pub2014; set t2.y2014; run;


/****************************** STEP 1: Physicians by specialty *****************************/

/* Create reduced dataset from 2014 Provider file with only NPI, ACO_NUM, and PRVDR_SPCLTY_CD.
...
proc sort data=pro2014_1; by ACO_NUM id PRVDR_SPCLTY_CD NPI;
proc transpose data=pro2014_1
			   out=pro2014_1
			   prefix=PHYS_SPEC; 
	by ACO_NUM id;
	id PRVDR_SPCLTY_CD;
	var NPI;
run;
*note: "16547 observations ommitted due to missing ID value" 
16547 rows represented practices or groups that weren't recorded a specialty, 
...
proc sql;
	create table pro2014_11 as
		select ACO_NUM, count(distinct PHYS_SPEC01) as PHYS_SPEC01, count(distinct PHYS_SPEC02) as PHYS_SPEC02,
			...
			count(distinct PHYS_SPECC3) as PHYS_SPECC3
	from pro2014_1
	group by ACO_NUM;
quit;
...
/* Create reduced dataset from 2014 Provider file with only TIN, NPI, ACO_NUM, and PRVDR_SPCLTY_CD.
   Also, create new ID var with which to transpose data in a couple of steps.*/
data pro2014_2;
	set pro2014;
...
proc sql;
	create table pro2014_2_temp as
		select * from pro2014_2 where NPI is not null;
quit;

/* use PROC TRANSPOSE to reshape data from a long format to a wide format. */
proc sort data=pro2014_2_temp; by ACO_NUM id PRVDR_SPCLTY_CD TIN;
proc transpose data=pro2014_2_temp
			   out=pro2014_2
			   prefix=GRP_SPEC; 
	by ACO_NUM id;
	id PRVDR_SPCLTY_CD;
...
proc sql;
	create table pro2014_21 as
		select ACO_NUM, count(distinct GRP_SPEC01) as GRP_SPEC01, count(distinct GRP_SPEC02) as GRP_SPEC02,
			...
			count(distinct GRP_SPECC3) as GRP_SPECC3
	from pro2014_2
	group by ACO_NUM;
quit;
...
proc sql;
	create table pro2014_31 as
		select ACO_NUM, count(distinct NPI) as RADs from pro2014_3 where dum_rad = 1
		group by ACO_NUM;
quit;																								

proc sql;
	create table pro2014_32 as
		select ACO_NUM, count(distinct NPI) as PCPs from pro2014_3 where dum_PCP = 1
		group by ACO_NUM;
quit;

proc sql;
	create table pro2014_33 as
		select ACO_NUM, count(distinct NPI) as NPs from pro2014_3 where dum_NP = 1
		group by ACO_NUM;
quit;																								

proc sql;
	create table pro2014_34 as
		select ACO_NUM, count(distinct NPI) as Others from pro2014_3 where dum_other = 1
		group by ACO_NUM;
quit;
...
proc sql;
	create table ACO_NUM_list as
		select distinct ACO_NUM from t1.provider_aco_ssp_2014;
quit;																	   	   

proc sort data=pro2014_31; by ACO_NUM; run;
proc sort data=pro2014_32; by ACO_NUM; run;
proc sort data=pro2014_33; by ACO_NUM; run;
proc sort data=pro2014_34; by ACO_NUM; run;
proc sort data=ACO_NUM_list; by ACO_NUM; run;
data pro2014_35;
	merge ACO_NUM_list pro2014_31 pro2014_32 pro2014_33 pro2014_34;
	by ACO_NUM;
run;
...
/****************************** STEP 5: 2014 Masterfile *****************************/

/* Combine the datasets for 2014 Masterfile */
proc sort data=pro2014_12; by ACO_NUM; run;
proc sort data=pro2014_21; by ACO_NUM; run;
proc sort data=pro2014_36; by ACO_NUM; run;
data t3.pro2014;
	merge pro2014_12 pro2014_21 pro2014_36;
	by ACO_NUM;
run;
