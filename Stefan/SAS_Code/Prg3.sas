...
data pro2015; set t1.provider_aco_ssp_2015; run;
data pub2015; set t2.y2015; run;


/****************************** STEP 1: Physicians by specialty *****************************/

/* Create reduced dataset from 2015 Provider file with only NPI, ACO_NUM, and PRVDR_SPCLTY_CD.
...
proc sort data=pro2015_1; by ACO_NUM id PRVDR_SPCLTY_CD NPI;
proc transpose data=pro2015_1
			   out=pro2015_1
			   prefix=PHYS_SPEC; 
	by ACO_NUM id;
	id PRVDR_SPCLTY_CD;
	var NPI;
run;
*note: "18626 observations ommitted due to missing ID value" 
18626 rows represented practices or groups that weren't recorded a specialty, 
...
proc sql;
	create table pro2015_11 as
		select ACO_NUM, count(distinct PHYS_SPEC01) as PHYS_SPEC01, count(distinct PHYS_SPEC02) as PHYS_SPEC02,
			...
			count(distinct PHYS_SPECC3) as PHYS_SPECC3, count(distinct PHYS_SPEC70) as PHYS_SPEC70, count(distinct PHYS_SPEC73) as PHYS_SPEC73
	from pro2015_1
	group by ACO_NUM;
quit;
...
proc sql;
	create table pro2015_12 as
		select *,       case when PHYS_SPEC01 > 0 then 1 else 0 end as DUM_PHYS_SPEC01,
						...
						case when PHYS_SPECC3 > 0 then 1 else 0 end as DUM_PHYS_SPECC3,
						case when PHYS_SPEC70 > 0 then 1 else 0 end as DUM_PHYS_SPEC70,
						case when PHYS_SPEC73 > 0 then 1 else 0 end as DUM_PHYS_SPEC73
	from pro2015_11
	group by ACO_NUM;
quit;
...
/* Create reduced dataset from 2015 Provider file with only TIN, NPI, ACO_NUM, and PRVDR_SPCLTY_CD.
   Also, create new ID var with which to transpose data in a couple of steps.*/
data pro2015_2;
	set pro2015;
...
proc sql;
	create table pro2015_2_temp as
		select * from pro2015_2 where NPI is not null;
quit;

/* use PROC TRANSPOSE to reshape data from a long format to a wide format. */
proc sort data=pro2015_2_temp; by ACO_NUM id PRVDR_SPCLTY_CD TIN;
proc transpose data=pro2015_2_temp
			   out=pro2015_2
			   prefix=GRP_SPEC; 
	by ACO_NUM id;
	id PRVDR_SPCLTY_CD;
...
proc sql;
	create table pro2015_21 as
		select ACO_NUM, count(distinct GRP_SPEC01) as GRP_SPEC01, count(distinct GRP_SPEC02) as GRP_SPEC02,
			...
			count(distinct GRP_SPECC3) as GRP_SPECC3, count(distinct GRP_SPEC70) as GRP_SPEC70, count(distinct GRP_SPEC73) as GRP_SPEC73
	from pro2015_2
	group by ACO_NUM;
quit;
...
proc sql;
	create table pro2015_31 as
		select ACO_NUM, count(distinct NPI) as RADs from pro2015_3 where dum_rad = 1
		group by ACO_NUM;
quit;																								

proc sql;
	create table pro2015_32 as
		select ACO_NUM, count(distinct NPI) as PCPs from pro2015_3 where dum_PCP = 1
		group by ACO_NUM;
quit;

proc sql;
	create table pro2015_33 as
		select ACO_NUM, count(distinct NPI) as NPs from pro2015_3 where dum_NP = 1
		group by ACO_NUM;
quit;																								

proc sql;
	create table pro2015_34 as
		select ACO_NUM, count(distinct NPI) as Others from pro2015_3 where dum_other = 1
		group by ACO_NUM;
quit;
...
proc sql;
	create table ACO_NUM_list as
		select distinct ACO_NUM from t1.provider_aco_ssp_2015;
quit;																	   	   

proc sort data=pro2015_31; by ACO_NUM; run;
proc sort data=pro2015_32; by ACO_NUM; run;
proc sort data=pro2015_33; by ACO_NUM; run;
proc sort data=pro2015_34; by ACO_NUM; run;
proc sort data=ACO_NUM_list; by ACO_NUM; run;
data pro2015_35;
	merge ACO_NUM_list pro2015_31 pro2015_32 pro2015_33 pro2015_34;
	by ACO_NUM;
run;
...
/****************************** STEP 5: 2015 Masterfile *****************************/

/* Combine the datasets for 2015 Masterfile */
proc sort data=pro2015_12; by ACO_NUM; run;
proc sort data=pro2015_21; by ACO_NUM; run;
proc sort data=pro2015_36; by ACO_NUM; run;
data t3.pro2015;
	merge pro2015_12 pro2015_21 pro2015_36;
	by ACO_NUM;
run;